...
//        }
    ...
    public CustomCardView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CustomCardView, defStyleAttr,
                R.style.CardView);
        ColorStateList backgroundColor;
        if (a.hasValue(R.styleable.CustomCardView_cardBackgroundColor)) {
            backgroundColor = a.getColorStateList(R.styleable.CustomCardView_cardBackgroundColor);
        } else {
            ...
            backgroundColor = ColorStateList.valueOf(hsv[2] > 0.5f
                    ? getResources().getColor(R.color.CustomCardView_light_background)
                    : getResources().getColor(R.color.CustomCardView_dark_background));
        }
        float radius = a.getDimension(R.styleable.CustomCardView_cardCornerRadius, 0);
        float elevation = a.getDimension(R.styleable.CustomCardView_cardElevation, 0);
        float maxElevation = a.getDimension(R.styleable.CustomCardView_cardMaxElevation, 0);
        mCompatPadding = a.getBoolean(R.styleable.CustomCardView_cardUseCompatPadding, false);
        mPreventCornerOverlap = a.getBoolean(R.styleable.CustomCardView_cardPreventCornerOverlap, true);
        int defaultPadding = a.getDimensionPixelSize(R.styleable.CustomCardView_contentPadding, 0);
        mContentPadding.left = a.getDimensionPixelSize(R.styleable.CustomCardView_contentPaddingLeft,
                defaultPadding);
        mContentPadding.top = a.getDimensionPixelSize(R.styleable.CustomCardView_contentPaddingTop,
                defaultPadding);
        mContentPadding.right = a.getDimensionPixelSize(R.styleable.CustomCardView_contentPaddingRight,
                defaultPadding);
        mContentPadding.bottom = a.getDimensionPixelSize(R.styleable.CustomCardView_contentPaddingBottom,
                defaultPadding);
        ...
        if (a.hasValue(R.styleable.CustomCardView_shadowStartColor)) {
            shadowStartColor = a.getColor(R.styleable.CustomCardView_shadowStartColor, 0);
        } else {
            shadowStartColor = getContext().getResources().getColor(R.color.CustomCardView_shadow_start_color);
        }

        if (a.hasValue(R.styleable.CustomCardView_shadowEndColor)) {
            shadowEndColor = a.getColor(R.styleable.CustomCardView_shadowEndColor, 0);
        } else {
            shadowEndColor = getContext().getResources().getColor(R.color.CustomCardView_shadow_end_color);
        }

        a.recycle();
        IMPL.initialize(mCardViewDelegate, context, backgroundColor,
    ...
    private final CardViewDelegate mCardViewDelegate = new CardViewDelegate() {
        ...
        public void setShadowPadding(int left, int top, int right, int bottom) {
            ...
            CustomCardView.super.setPadding(left + mContentPadding.left, top + mContentPadding.top,
        ...
        public void setMinWidthHeightInternal(int width, int height) {
            if (width > mUserSetMinWidth) {
                CustomCardView.super.setMinimumWidth(width);
            }
            if (height > mUserSetMinHeight) {
                CustomCardView.super.setMinimumHeight(height);
            }
        }
